...
- 업무 등록, 검색, 유형 필터, 수정, 완료 처리, 삭제
- 삭제한 업무를 `삭제 목록` 시트로 이동
- 안내사항 등록, 수정, 삭제
- PC·모바일 반응형 화면
...
삭제 작업은 원본 행을 바로 없애기 전에 `삭제 목록` 시트로 복사합니다. 다만 동시에 여러 사람이 같은 행을 수정하면 행 번호가 바뀔 수 있으므로, 수정·삭제 직전에는 새로고침하는 것이 안전합니다.
